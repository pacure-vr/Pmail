...
(function () {
  ...
  const lan = new window.PmailLanConnector('http://localhost:7890');
  ...
  async function init() {
    ...
    // Intentar conectar con la app de PC en la red local
    await checkLanConnection();

    // Cargar datos locales desde IndexedDB si la app de PC no está activa
    await loadInitialData();
  }

  async function checkLanConnection() {
    const status = await lan.checkStatus();
    if (status.connected) {
      dom.btnLanStatus.className = 'px-2 py-0.5 rounded text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center gap-1 cursor-pointer';
      dom.btnLanStatus.innerHTML = '<span class="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span> LAN: PC Conectada';

      // Sincronizar correos desde la PC
      try {
        const pcEmails = await lan.fetchEmails();
        if (pcEmails.length > 0) {
          state.emails = pcEmails;
          await db.saveEmailsBatch(pcEmails);
          renderEmailList();
          updateBadges();
        }
      } catch (err) {
        console.warn('Error sincronizando emails desde PC:', err);
      }

      // Escuchar eventos en vivo SSE
      lan.connectEvents((event) => {
        if (event.type === 'email_queued' || event.type === 'email_sent') {
          checkLanConnection();
        }
      });
    } else {
      dom.btnLanStatus.className = 'px-2 py-0.5 rounded text-[10px] bg-amber-500/10 text-amber-400 border border-amber-500/20 flex items-center gap-1 cursor-pointer';
  ...
  async function loadInitialData() {
    ...
    if (localEmails.length === 0 && !lan.isConnected) {
      // Sembrar datos iniciales demostrativos
      localEmails = [
        {
          id: 'web_welcome_01',
          from_address: 'soporte@pac.p',
          to_address: 'usuario@pac.p',
          subject: 'Bienvenido al Cliente Web de Pmail',
          body_text: 'Este cliente web funciona de forma autónoma con IndexedDB y puede conectarse a tu PC en la red local.',
          body_html: '<p>Este cliente web funciona de forma autónoma con <b>IndexedDB</b> y puede conectarse a tu PC en la red local.</p>',
          folder: 'inbox',
          status: 'SYNCED',
  ...
  function setupEventListeners() {
    ...
    dom.btnSubmitSend.addEventListener('click', async () => {
      ...
      try {
        if (lan.isConnected) {
          // Despachar a través del servidor SMTP de la PC
          await lan.sendEmail(emailData);
          alert('Correo enviado a la cola del Host PC.');
        } else {
          // Guardar localmente en IndexedDB
          emailData.status = 'PENDING_SEND';
          await db.saveEmail(emailData);
          state.emails.push(emailData);
          alert('Sin conexión LAN: Guardado en IndexedDB como Pendiente de Envío.');
        }
        renderEmailList();
        updateBadges();
        closeComposer();
    ...
    // Modal LAN
    dom.btnLanStatus.addEventListener('click', () => dom.lanModal.classList.remove('hidden'));
    dom.btnLanSettings.addEventListener('click', () => dom.lanModal.classList.remove('hidden'));
    dom.btnCloseLan.addEventListener('click', () => dom.lanModal.classList.add('hidden'));

    dom.btnTestLan.addEventListener('click', async () => {
      lan.setServerUrl(dom.lanUrlInput.value.trim());
